...
if version < 3652 then -- All versions below 3652 don't have buildin global icon support, so we need to insert the icons by our own function
    LOG('Nuclear Repulsor Shields: [uiutil.lua '..debug.getinfo(1).currentline..'] - Gameversion is older then 3652. Hooking "UIFile" to add our own unit icons')
...
local MyUnitIdTable = {
   ...
   qeb4409=true, -- EXSD Puls: Belmont - (City Shield Generator)
    ...
    LOG('Nuclear Repulsor Shields: [uiutil.lua '..debug.getinfo(1).currentline..'] - Gameversion is 3652 or newer. No need to insert the unit icons by our own function.')
end